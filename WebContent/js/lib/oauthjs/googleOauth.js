...
angular.module('googleOauth', ['angularOauth']).
  ...
  constant('GoogleUserInfo', function(config, accessToken) {
	    var $injector = angular.injector(['ng']);
	    return $injector.invoke(['$http', '$rootScope', '$q', function($http, $rootScope, $q) {
	      var deferred = $q.defer();
	      ...
	      $rootScope.$apply(function() {
	        $http({method: 'GET', url: userInfoEndpoint, params: {access_token: accessToken}}).
	          success(function(data) {
	            if (data.sub) {
	              deferred.resolve(data);
	            } else {
	              deferred.reject({name: 'Not able to fetch information'});
	            }
	          }).
	          error(function(data, status, headers, config) {
	            deferred.reject({
  ...
  config(function(TokenProvider, GoogleTokenVerifier, GoogleUserInfo) {
