...
angular.module('angularOauth', []).
    ...
    this.$get = function($q, $http, $window, $rootScope) {
      ...
      return {
        ...
        userInfo: function(accessToken) {
            return config.userInfo(config, accessToken);
          },
        ...
        getTokenByPopup: function(extraParams, popupOptions) {
          ...
          if(mobileAndTabletcheck()){
        	  popup = cordova.InAppBrowser.open(url, popupOptions.name, formatPopupOptions(popupOptions.openParams));        	  
          }else{
        	  popup = window.open(url, popupOptions.name, formatPopupOptions(popupOptions.openParams));
          }
          ...
          addEvent(window, 'storage', function (event) {
        	  if (event.key == "appaccessToken" && event.newValue) {
        		  popup.close();
        		  var paramsObj = JSON.parse(event.newValue);
        		  localStorage.removeItem("appaccessToken");
        		  if(paramsObj.access_token){
        			  $rootScope.$apply(function() {
        				  deferred.resolve(paramsObj);
            		  });
        		  }else{
        			  $rootScope.$apply(function() {
        				  deferred.reject(paramsObj);
        			  });
        		  }
        	  }
          });
  ...
  controller('CallbackCtrl', function($scope, $location) {
    ...
    localStorage["appaccessToken"] = JSON.stringify(params);
    
    //window.opener.postMessage(params, "*");
    //window.close();
  })
  .controller('CallbackCtrl2', function($scope, $location) {
	    ...
	    window.location="http://jantakhabar-myspring.rhcloud.com/INFOAPP/";
	    //window.opener.postMessage(params, "*");
	  });
